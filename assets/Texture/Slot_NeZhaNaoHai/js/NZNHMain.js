...
cc.Class({
    ...
    startBigWin() {
        ...
        this.node.runAction(cc.sequence(cc.delayTime(3), cc.callFunc(()=>{
            ...
            this.node.getChildByName("longwang1").active = false;
            this.bigWinNode.getChildByName("longwang").getComponent(sp.Skeleton).setAnimation(0, "animation2", true);
        })));
        for (let i = 1; i < 7; i++) {
    ...
    bigWinChoose (event, customentData) {
        if (this.bigWinBoo == false) {
            return;
        }
        this.bigWinBoo = false;
        let tempBeishu = [20, 1, 5, 50, 2, 10];
        let beishu = [];
        for (let i = 0; i < tempBeishu.length; i++) {
        ...
        for (let i = 1; i < 7; i++) {
            if (i == customentData) {
                this.bigWinNode.getChildByName("beike" + i).getComponent(sp.Skeleton).setAnimation(0, "ami3", false);
                this.bigWinNode.getChildByName("beike" + i).getChildByName("shuzi_yellow").active = true;
                this.bigWinNode.getChildByName("beike" + i).getChildByName("shuzi_yellow").getComponent(cc.Label).string = "x" + this.bigWinCard;
            } else {
                this.bigWinNode.getChildByName("beike" + i).getComponent(sp.Skeleton).setAnimation(0, "ami3a", false);
                this.bigWinNode.getChildByName("beike" + i).getChildByName("shuzi_Blue").active = true;
                this.bigWinNode.getChildByName("beike" + i).getChildByName("shuzi_Blue").getComponent(cc.Label).string = "x" + beishu.pop();
            }
        }
        this.bigWinNode.getChildByName("longwang").getComponent(sp.Skeleton).setAnimation(0, "animation3", true);
        this.bigWinNode.runAction(cc.sequence(cc.delayTime(3), cc.callFunc(()=>{
            this.audio.playBgm(0);
            this.bigWinNode.active = false;
