const NETPATH = 'http://60.205.191.87';
//const NETPATH = 'http://192.168.18.7';
    ...
    function Single() {
        ...
        this.startGameFunction = function () {
            ...
            self.LandlordsSocket.on("OpenWinResult", function (ret) {
                var result = self.changeResultJSON_Function(ret);
                console.log(ret);
                if (window.longhudou_ins)
                    window.longhudou_ins.showResult(result);
                console.log("～～～～～～开牌");
            });
